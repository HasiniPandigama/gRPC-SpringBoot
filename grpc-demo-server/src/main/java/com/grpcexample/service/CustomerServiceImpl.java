...
public class CustomerServiceImpl extends CustomerServiceGrpc.CustomerServiceImplBase {
    ...
    @Override
    public void getCustomerAccounts(CustomerDetails request, io.grpc.stub.StreamObserver<CustomerAccountDetails> responseObserver) {
    ...
    public StreamObserver<CustomerAccountDetails> getAccountsOlderThanOneYear(StreamObserver<CustomerAccountDetails> responseObserver) {
        return new StreamObserver<CustomerAccountDetails>() {
            CustomerAccountDetails oldestAccount = null;

            long secondsPerYear = 31536000;
            long yearAgoInSecs = Instant.now().getEpochSecond() - secondsPerYear;

            List<CustomerAccountDetails> olderAccounts = new ArrayList<>();
            @Override
